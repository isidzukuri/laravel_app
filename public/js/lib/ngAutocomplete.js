...
.factory('ngAutocompleteService', ['$http', function($http) {
	...
	self.getData = function (url, keyword) {
		...
		return $http.get(url+"/"+keyword, { query: keyword });
	};
...
.directive('ngAutocomplete', ['$timeout', '$filter', 'ngAutocompleteService', function($timeout, $filter, ngAutocompleteService) {
	...
	return {
		...
		link: function(scope, element, attrs, ngModel) {
			...
			scope.selectResult = function (r) {
				if(r){
					...
					ngModel.$setViewValue(scope.keyword)
				}
			};
		...
		template:
		'<input type="text" ng-model="keyword" class="ng-autocomplete-input form-control" placeholder="{{ placeholder }}" ng-change="getResults()" ng-keyup="keyupHandler($event)" ng-blur="blurHandler()" ng-focus="currentIndex = 0" autocorrect="off" autocomplete="off">' +
